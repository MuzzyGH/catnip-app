...
<p>If you didn't create this account, please ignore this email.</p>
    ...
    def send_password_reset_email(self, email: str, token: str) -> bool:
        """
        Send password reset email using Resend (preferred) or SMTP fallback
        """
        ...
        if self.resend_api_key:
            try:
                ...
                result = resend.Emails.send({
                    "from": self.from_email,
                    "to": email,
                    "subject": "Reset Your Catnip Password",
                    "html": f"""
                    <html>
                    <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                        <h2 style="color: #333;">Reset Your Password</h2>
                        <p>Click the button below to reset it (link valid 1 hour):</p>
                        <p style="text-align: center; margin: 30px 0;">
                            <a href="{reset_link}"
                               style="background-color: #4CAF50; color: white; padding: 12px 24px; 
                                      text-decoration: none; border-radius: 5px; display: inline-block;">
        ...
        if self.smtp_user and self.smtp_password:
            try:
                ...
                msg = MIMEMultipart('alternative')
                msg['Subject'] = 'Reset Your Catnip Password'
                msg['From'] = self.from_email
                msg['To'] = email

                text = f"""Reset your Catnip password (valid 1 hour):
{reset_link}
"""
...
<p>Click the link below to reset it (valid 1 hour):</p>
